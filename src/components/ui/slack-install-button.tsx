...
export function SlackInstallButton({
  location,
  label = "Add to Slack - It's Free",
  className = "",
}: SlackInstallButtonProps) {
  return (
    <a
      href={SIGNUP_URL}
      onClick={() => trackInstallClick(location)}
      className={`inline-flex items-center gap-2.5 rounded-lg bg-uply-green px-7 py-3.5 text-sm font-semibold text-uply-dark transition-all duration-200 hover:brightness-110 hover:shadow-[0_0_24px_-4px_rgba(104,239,63,0.4)] ${className}`}
    >
